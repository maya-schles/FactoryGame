...
public class GameTile implements MapTile {
    ...
    public static GameTile[] fromArr(TileType[] tileTypes) {
        GameTile[] res = new GameTile[tileTypes.length];
        for(int i = 0; i < tileTypes.length; i++) {
            res[i] = new GameTile(tileTypes[i]);
        }
        return res;
    }

    public static GameTile[][] from2dArr(TileType[][] tileTypes) {
        GameTile[][] res = new GameTile[tileTypes.length][tileTypes[0].length];
        for(int i = 0; i < tileTypes.length; i++) {
            res[i] = fromArr(tileTypes[i]);
        }
        return res;
    }
